...
void AEyeXActorBase::UpdateDwellTime(float DeltaSeconds)
{
	if (bTraceHit && IsWithinDistance())
	{
		...
		{
			ClampTowardsTarget(CurrentTimeOut, -DeltaSeconds, DwellTimeDeactivate);
		}
	}
	else
	{
		...
		{
			ClampTowardsTarget(CurrentTimeIn, -DeltaSeconds, DwellTimeActivate);
		}
	}
}
...
bool AEyeXActorBase::ClampTowardsTarget(float& CurrentTime, const float DeltaSeconds, const float Target)
{
	...
	if (DeltaSeconds > 0 && CurrentTime == Target)
		return true;
	else if (DeltaSeconds < 0 && CurrentTime == 0.0f)
		return true;
